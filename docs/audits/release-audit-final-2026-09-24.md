...
| Media and retention | Signed conditional creation, bounded integrity reads/downloads, stalled PUT recovery, encrypted screen-lifetime cancellation; photo/voice/video preparation and teardown tests. Candidate extends cancellation to legacy conversations and covers both transports with 16 local passing cases. Orphans are selected after 24h in batches and object deletion precedes row deletion. | REL-006–010 encrypted-flow fixes verified in preceding PRs; legacy parity awaits current CI. Linked media has no newly introduced automatic deletion. Provider migration, quotas/tiering and physical quality acceptance remain open. |
| Log and trace privacy | Four real ASGI request-middleware tests; installed FastAPI OpenTelemetry instrumentation reproduction; trace export regression with synthetic URLs, query, database statements, exception details and custom metadata; isolated subprocess exercising the configured structured/standard logging pipeline. | REL-011: raw query values entered trace export. REL-012: exception text and nested sensitive fields bypassed log redaction. Candidate filters exported traces, retains exception class without details and redacts nested PIN/token fields. Historical logs/traces are not retroactively scrubbed. |
...
## REL-012 — exception and nested log fields

An isolated reproduction through `configure_structured_logging` confirmed a synthetic exception value was serialized into the log. The existing top-level redactor ran before exception formatting and did not inspect nested dictionaries. The candidate strips exception/stack rendering input while preserving the exception class, recursively redacts sensitive keys including PIN/unlock headers and does not mutate nested caller dictionaries. A subprocess regression covers both structlog and standard-library exception logging plus nested fields. Less diagnostic detail is retained deliberately. Arbitrary private data embedded in an unstructured event string cannot be recognized generically; callers must continue to avoid logging bodies, raw URLs or private values as message text.

The installed FastAPI metric instrumentation was separately exercised with synthetic contact-query and unmatched-path values; neither appeared in its in-memory metric result. This is limited evidence for those inputs/configuration, not a certification of every third-party exporter or operator-added metric.
