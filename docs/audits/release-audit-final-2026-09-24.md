...
| Operations and recovery | Non-root runtime and capability restrictions; private MinIO, explicit production origin/E2EE gate; backup manifest/checksums and exclusive maintenance lock; restore stops writers and leaves failure offline; Linux fault injection and pinned images in CI. | Mocked failure handling is covered; no actual DB/object restore, recovery-time measurement, off-host backup verification or server hardening audit performed. |
...
4. **Recovery drill:** restore synthetic database and encrypted objects together in an isolated environment, verify reads and measure RTO/RPO; verify encrypted independent backups and key-recovery limitations. No production restore is authorized.
