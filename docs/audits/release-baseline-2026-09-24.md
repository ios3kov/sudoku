...
## Technical audit pass 8 — deterministic repository security gate
...
**REL-013 — Medium, automated verification passed:** MLS KeyPackage device discovery and single-use claim endpoints accepted any authenticated `user_id`. An unrelated authenticated account could enumerate another user's active secure-device metadata and consume available KeyPackages, causing availability degradation for future MLS setup. This did not expose message plaintext or private MLS state. Access is now limited to the same user, a synced contact, or a user sharing an active conversation; phone-migration compatibility follows the existing contact policy. A regression verifies an unrelated authenticated phone account receives 403 for both device listing and KeyPackage claim and cannot consume the target package.


**REL-014 — Medium, automated verification passed:** MLS `device_add` / `device_remove` membership-change finalization deliberately allowed a participant other than the original requester to complete rekey choreography, but it did not require that caller to belong to the affected conversation. A logged-in outsider who learned a transition UUID could therefore finalize a foreign device transition. Finalization now requires an active conversation membership for device rekeys; ordinary member add/remove transitions retain their requester check. A regression verifies an unrelated authenticated user receives 403 and the transition remains pending.


**REL-015 — Medium, automated verification passed:** account login performed Argon2 verification synchronously inside an async FastAPI handler and skipped password verification entirely for unknown/inactive users. This created an event-loop blocking surface under login load and a password-verification timing difference that could aid account enumeration. Login now always performs one Argon2 verification using either the real hash or a process-local dummy hash and executes it in FastAPI's threadpool. Other request-time password hash/verify operations in the auth route were moved to the threadpool as well. A regression verifies an unknown phone login still invokes the dummy verifier before returning 401.


2026-09-25 reconciliation: MLS discovery, rekey ACL and login hardening are REL-013/014/015; REL-011/012 remain trace/log privacy findings in the consolidated audit. Their automated verification is included in the [candidate CI](release-candidate-2026-09-25.md). Historical pending statements are retained as dated evidence, not current status.
