...
A subsequent screenshot showed a faint shaded seam above the bottom safe area. Disabled UIKit's automatic scroll-edge effects on iOS 26+ for the full-screen web canvas, which has no native overlay bars requiring edge separation. The SDK availability guard preserves older iOS support. Simulator build passed and the updated device build launched; visual confirmation remains pending.

The operator confirmed the faint strip persists after disabling native scroll-edge effects and is also visible on messenger login. That hypothesis did not resolve the visual defect. Do not mark the bottom-seam check complete. A proposed idle-Sudoku shadow override was not shipped because it would not account for the login-screen evidence. Further diagnosis requires an unlocked operator Mac/device.


Further on-device inspection identified two causes: the idle Sudoku `::after` retained a downward CSS shadow, and login's white canvas stopped above a 34-point beige native bottom safe area. The native web canvas now extends to the window bottom while preserving top/notch constraints; safe-area CSS padding protects bottom controls. The idle shadow is disabled except during the deliberate reveal gesture. A main-frame, production-origin-only compatibility stylesheet supplies these fixes to the currently hosted web release without a production deployment. Xcode confirmed the revised development build running on the physical iPhone; operator verification on both screens remains pending. Temporary layout diagnostics were removed.
