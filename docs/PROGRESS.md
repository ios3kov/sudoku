...
### Step 13 — Search + conversation preferences

- Membership-gated message text search implemented.
- Per-user pin/unpin and notification mute/unmute implemented.
- Pinned conversations sort first for that user.
- Muted conversations are excluded from server Web Push recipients without affecting realtime/durable delivery.
- Mobile search and conversation-preferences UI implemented.
- Integration harness extended for search and preference persistence.
...
Install all declared dependencies in a network-enabled runner and execute migrations + real-service API integration tests + Next production build. Fix any runtime failures before deployment; do not claim production readiness until those checks pass.
