...
The native application is portrait-only except for landscape video playback. [Step99](steps/99-private-video-playback.md) records viewer cleanup, origin checks, browser fallback and orientation policy. Physical-iPhone, VoiceOver/Dynamic Type and audiovisual-quality acceptance remain open. The operator Mac now has Xcode 27.0 (27A266a), iOS 27.0 platform support and XcodeGen 2.46.0. Local simulator compilation succeeded for source `3c016d025418d21701bb9af8c1aa6065a78c907b`; all 8 native tests passed in Xcode on the dedicated iOS 27.0 simulator. The connected iPhone 12 mini (iOS 26.0) has Developer Mode enabled, and the development build is installed and running under Xcode after Apple Account sign-in, Personal Team signing and operator confirmation of certificate trust. Visual/functional acceptance remains pending. Physical-device acceptance was not performed. The host still loads the production web origin; simulator unit tests do not validate the unreleased web changes on a real device. Russian storage provisioning/migration and production/store release remain separately gated. Production was not changed.
