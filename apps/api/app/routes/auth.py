...
_DUMMY_PASSWORD_HASH = hash_password("sudoku-login-timing-dummy-password")
...
async def login(payload: LoginRequest, request: Request, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    candidate_hash = user.password_hash if user is not None and user.status == "active" else _DUMMY_PASSWORD_HASH
    password_ok = await run_in_threadpool(verify_password, candidate_hash, payload.password)
    succeeded = bool(user is not None and user.status == "active" and password_ok)
    db.add(
        LoginAttempt(
...
async def update_display_name(
    ...
    auth.user.profile_setup_completed = True
    db.add(
        AuditEvent(
            actor_user_id=auth.user.id,
