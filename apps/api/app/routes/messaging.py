...
async def serialize_message(db: AsyncSession, message: Message) -> dict:
    ...
    reaction_rows = (
        await db.execute(
            select(MessageReaction.emoji, MessageReaction.user_id).where(MessageReaction.message_id == message.id)
        )
    ).all()
    ...
    return {
        ...
        "reactions": [{"emoji": emoji, "user_ids": user_ids} for emoji, user_ids in grouped_reactions.items()],
    }
...
async def conversation_members_response(db: AsyncSession, conversation_id: uuid.UUID) -> list[ConversationMemberResponse]:
    rows = (
        await db.execute(
...
async def conversation_response(
    db: AsyncSession, conversation: Conversation, membership: ConversationMember
) -> ConversationResponse:
...
async def require_membership(db: AsyncSession, conversation_id: uuid.UUID, user_id: uuid.UUID) -> ConversationMember:
    member = (
        await db.execute(
