...
test("admin invite create, copy and close buttons perform their actions", async ({ page }) => {
  ...
  await page.getByLabel("Email (optional)").fill("member@example.test");
  ...
  expect(await page.evaluate(() => window.__buttonAudit.calls.invites.at(-1))).toBe("member@example.test");
